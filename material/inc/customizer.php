...
function material_customize_register( $wp_customize ) {
	$color_scheme = material_get_color_scheme();
	...
	$wp_customize->add_setting( 'color_scheme', array(
		'default'           => 'default',
		'sanitize_callback' => 'material_sanitize_color_scheme',
		'transport'         => 'postMessage',
	) );

	$wp_customize->add_control( 'color_scheme', array(
		'label'    => __( 'Base Color Scheme', 'material-blog-story' ),
		'section'  => 'colors',
		'type'     => 'select',
		'choices'  => material_get_color_scheme_choices(),
		'priority' => 1,
	) );
	...
	$wp_customize->add_control( new WP_Customize_Color_Control( $wp_customize, 'sidebar_textcolor', array(
		'label'       => __( 'Header and Sidebar Text Color', 'material-blog-story' ),
		'description' => __( 'Applied to the header on small screens and the sidebar on wide screens.', 'material-blog-story' ),
		'section'     => 'colors',
	) ) );
	...
	$wp_customize->add_control( new WP_Customize_Color_Control( $wp_customize, 'header_background_color', array(
		'label'       => __( 'Header and Sidebar Background Color', 'material-blog-story' ),
		'description' => __( 'Applied to the header on small screens and the sidebar on wide screens.', 'material-blog-story' ),
		'section'     => 'colors',
	) ) );

	// Add an additional description to the header image section.
	$wp_customize->get_section( 'header_image' )->description = __( 'Applied to the header on small screens and the sidebar on wide screens.', 'material-blog-story' );
}
add_action( 'customize_register', 'material_customize_register', 11 );


function material_get_color_schemes() {
	return apply_filters( 'material_color_schemes', array(
		'default' => array(
			'label'  => __( 'Default', 'material-blog-story' ),
			'colors' => array(
				'#f1f1f1',
				'#ffffff',
				'#ffffff',
		...
		'dark'    => array(
			'label'  => __( 'Dark', 'material-blog-story' ),
			'colors' => array(
				'#111111',
				'#202020',
				'#202020',
		...
		'yellow'  => array(
			'label'  => __( 'Yellow', 'material-blog-story' ),
			'colors' => array(
				'#f4ca16',
				'#ffdf00',
				'#ffffff',
		...
		'pink'    => array(
			'label'  => __( 'Pink', 'material-blog-story' ),
			'colors' => array(
				'#ffe5d1',
				'#e53b51',
				'#ffffff',
		...
		'purple'  => array(
			'label'  => __( 'Purple', 'material-blog-story' ),
			'colors' => array(
				'#674970',
				'#2e2256',
				'#ffffff',
		...
		'blue'   => array(
			'label'  => __( 'Blue', 'material-blog-story' ),
			'colors' => array(
				'#e9f2f9',
				'#55c3dc',
				'#ffffff',
...
if ( ! function_exists( 'material_get_color_scheme' ) ) :

function material_get_color_scheme() {
	$color_scheme_option = get_theme_mod( 'color_scheme', 'default' );
	$color_schemes       = material_get_color_schemes();
...
if ( ! function_exists( 'material_get_color_scheme_choices' ) ) :

function material_get_color_scheme_choices() {
	$color_schemes                = material_get_color_schemes();
...
endif; // material_get_color_scheme_choices

if ( ! function_exists( 'material_sanitize_color_scheme' ) ) :

function material_sanitize_color_scheme( $value ) {
	$color_schemes = material_get_color_scheme_choices();
...
function material_color_scheme_css() {
	$color_scheme_option = get_theme_mod( 'color_scheme', 'default' );
	...
	$color_textcolor_rgb         = material_hex2rgb( $color_scheme[3] );
	$color_sidebar_textcolor_rgb = material_hex2rgb( $color_scheme[4] );
	$colors = array(
	...
	$color_scheme_css = material_get_color_scheme_css( $colors );

	wp_add_inline_style( 'material-style', $color_scheme_css );
}
add_action( 'wp_enqueue_scripts', 'material_color_scheme_css' );


function material_customize_control_js() {
	...
	wp_localize_script( 'color-scheme-control', 'colorScheme', material_get_color_schemes() );
}
add_action( 'customize_controls_enqueue_scripts', 'material_customize_control_js' );


function material_customize_preview_js() {
	wp_enqueue_script( 'material-customize-preview', get_template_directory_uri() . '/js/customize-preview.js', array( 'customize-preview' ), '20141216', true );
}
add_action( 'customize_preview_init', 'material_customize_preview_js' );


function material_get_color_scheme_css( $colors ) {
...
function material_color_scheme_css_template() {
	$colors = array(
	...
	<script type="text/html" id="tmpl-twentyfifteen-color-scheme">
		<?php echo material_get_color_scheme_css( $colors ); ?>
	</script>
	<?php
}
add_action( 'customize_controls_print_footer_scripts', 'material_color_scheme_css_template' );
